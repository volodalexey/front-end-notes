В статье я попытаюсь описать табличную разметку. Используют ли её сейчас.
Дам пример моей таблицы и разметки.
...
Теперь когда содержимое таблицы растянулось, ширина всей таблицы (примерно на моем экране) = `1836px`, колонки `Company` = `982px`, колонки `Contact` = `551px`.<br />
...
Так мы ломаем автовычисление и теперь таблица слушается заданных ширин для каждого столбца (или всей таблицы), но зато таблица точно вписывается в указанную ширину.<br />
...
Внутри него мы хотим показывать/вписать таблицу, если ширина таблицы больше чем контейнер, тогда надо показывать скролл для контейнера.
