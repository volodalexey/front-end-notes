...
Можно задать ограниченную высоту телу таблицы. Предполагая высоту родительского контейнера [можно попробовать](https://codepen.io/volodalexey/pen/dRWoQY).<br />
В результате мы ломаем табличное отображение тела таблицы CSS свойством `display: block`, а также нам необходимо синхронизировать прокрутку шапки с телом таблицы.
...
Интересный способ использовать дополнительный проход браузера.<br />
...
Все составные таблицы (и моя будущая тоже) страдают одним недостающим фактором, у них нет стандарта как их кастомизировать/настраивать (и это логично, т.к. отказались от HTML4 таблицы).<br /> 
...
Т.е. на свою таблицу вы будете тратить 2 дня на старт, зато потом в течении 30 мин кастомизировать, или взять готовую фреймворк-таблицу за 30 мин и потом кастомизировать каждую фичу за 1 день.

К премеру я покажу как сделать свою составную таблицу на React.<br />
Таблица будет:
...
```
Но есть одна проблема, т.к. браузер (не все браузеры) интерпретирует пустые места между ячейками как текстовые ноды.<br />
...
Элемент `TableColumn` "фейковый", он никогда не будет отображаться, а вот его содержимое `this.props.children` - будет отображаться в ячейке шапки.
...
```
Тело таблицы будет строить ячейки тоже на основе обработанного описания колонок:
```jsx harmony
...
```
Тело таблицы использует описания у которых есть свойство `dataField` поэтому необходимо отфильтровать описания используя функцию `getCellDescriptions`.<br />
...
Внутрь `TableColumn` можно вставлять любую разметку. По факту нужны такие варианты: текст, кнопка сортировки и кнопка фильтрации.
...
```
Компонент сортировки `SortButton` и компонент фильтрации `MultiselectDropdown` при изменении "выбрасывает" новые активные фильтры/сортировки, которые разработчик должен заменить в состоянии.<br />
...
- сортировка таблицы (множественная сортировка - опционально)
- фильтрация таблицы (множественная фильтрация - опционально)

Все это я реализовал в примере. Надеюсь теперь при переезде на новый фреймворк у вас как минимум появиться возможность не брать готовую, а сделать свою.
