В статье я покажу стандартную табличную разметку. Покажу какие есть у неё альтернативы. Дам пример моей таблицы и разметки, а также общие моменты.
...
```
Возникает вполне логичный вопрос, а зачем тогда вообще использовать тег `<table>`, если используется только автоподстройка ширины из оригинальной таблицы?<br />
...
Дальше будет объяснение только некоторых аспектов разработки, поэтому даю [сразу посмотреть результат](https://volodalexey.github.io/front-end-notes/).<br />
...
Все обязательные свойства я мог бы описать через `propTypes`, но после того [как их вынесли](https://facebook.github.io/react/blog/#new-deprecation-warnings) в отдельную библиотеку - это решение кажется сомнительным.<br />
Обязательно указать `row` - число, которое показывает индекс строки в шапке (если шапка будет группироваться).<br />
...
Все это я реализовал в примере. Надеюсь теперь при переходе на новый фреймворк у вас как минимум появился выбор - брать готовую или сделать свою.
